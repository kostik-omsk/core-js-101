...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startStr}${startNum}, ${endNum}${endStr}`;
}
...
function getDigitalRoot(num) {
  ...
  const sum = numArr.reduce((acc, val) => acc + val, 0);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (open.includes(bracket)) {
      stack.push(bracket);
    } else if (clos.includes(bracket)) {
      const matching = open[clos.indexOf(bracket)];
...
function toNaryString(num, n) {
  ...
  while (number !== 0) {
    const remainder = Math.abs(number % n).toString();
    res = remainder + res;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathsArr[0].length; i += 1) {
    const currentPath = pathsArr[0][i];

    if (!pathsArr.every((path) => path[i] === currentPath)) {
      return commonPath;
    }
...
function evaluateTicTacToePosition(pos) {
  ...
  const win = lines.find((line) => (line[0] && line[0] === line[1] && line[1] === line[2]));
